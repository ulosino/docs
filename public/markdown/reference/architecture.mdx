---
name: "The ULOSINO Architecture"
summary: "Technical information about how ULOSINO works under the hood."
date: "2021-04-12"
---
...
> **Note:** There are no `.jsx` files. All React component files use TypeScript.
...
> **Note:** When working in development mode, step 5 isn't applicable, as pages aren't being prerendered and we are using just-in-time compilation.
...
> **Note:** The automatically generated `node_modules` and `.next` folders are excluded by our `.gitignore`.
...
This powers Preferences, background updates, and other features.

---

## Performance Optimisation with Streaming and Edge Runtime

ULOSINO extensively takes advantage of the `<Suspense>` component introduced in React 18. It allows us to load everything within a "Suspense boundary" _after_ the page itself has loaded. This is particularly useful for our Search functions, which exceed `100kb` in size. The fallback is a loading spinner or a "Communicating with Server" `<Text>`.

We're also moving some of our code over to the Edge using Edge Runtime. This is fully-streaming based, using modern, clean, web APIs. This is powered by Edge Functions on Vercel.

Performance is a real focus at ULOSINO and we're continuing to investigate new solutions and approaches.
...
> **Tip:** ULOSINO includes a few custom error codes. Read through the [Error Reference](/docs/reference/errors) for more details.
...
This is the grand provider. It wraps around every page. `ApplicationProvider` provides `ChakraProvider`, keyboard shortcut handling, error handling, troubleshooting, and other functions.

> **Note:** Multiple technical limitations arise when attempting to use `ApplicationProvider` features in `.app_tsx`.
